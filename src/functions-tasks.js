...
function logger(func, logFunc) {
  return (...arg) => {
    const startArgs = arg.map((el) => JSON.stringify(el)).join(',');
    const funcStart = `${func.name}(${startArgs}) starts`;
    logFunc(funcStart);
    const res = func(...arg);
    const funcEnd = `${func.name}(${startArgs}) ends`;
    logFunc(funcEnd);
